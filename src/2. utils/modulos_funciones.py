...
def _train_neural_network(X_train, y_train, X_test, y_test):
...
# Convierte el modelo de Keras en un clasificador de scikit-learn utilizando KerasClassifier
nn_classifier = KerasClassifier(build_fn=_train_neural_network, 
...
def _ensemble_model(rf_model, knn_model, gbt_model,xgb_model, nn_classifier, X_train, y_train, X_test, y_test):
    
    # Create a dictionary of our models
    estimators=[('knn', knn_model), ('rf', rf_model), ('gbt', gbt_model),('nn',nn_classifier),('Xgb',xgb_model)]
...
ensemble_model = _ensemble_model(rf_model, knn_model, gbt_model,nn_classifier,xgb_model, X_train, y_train, X_test, y_test)
...
# Save the model in a file named model_fecha
with open("src/model/model_{}.pickle".format(fecha), 'wb') as f:
